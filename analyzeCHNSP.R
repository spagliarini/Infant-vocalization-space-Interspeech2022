...
###############
# UMAP
lmeAnova = lmer(CHNSPentropy_UMAP ~ AgeGroup + (1|ChildID), data = ourdata)
anova(lmeAnova)
summary(lmeAnova)

#Can't run with polynomial random effect b/c we don't have enough data points
#lmePoly = lmer(CHNSPentropy ~ poly(AGE,2) + (poly(AGE,2)|ChildID), data = ourdata)
#summary(lmePoly)
#confint(lmePoly)
...
predEntropy = predict(lmPoly)
ix = sort(ourdata$AGE,index.return=T)$ix

lines(ourdata$AGE[ix],predEntropy[ix])
save(predEntropy, file="entropy.Rdata")
